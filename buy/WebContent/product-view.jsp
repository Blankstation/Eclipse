...
<div id="main" class="wrap">
	...
	<%  int id = Integer.parseInt(request.getParameter("epid"));
		...
		List<Product> product = productDao.getAllProduct(id);
	...
	%>
		...
		<h1><%=p.getEpName() %></h1>
		<div class="infos">
			<div class="thumb"><img src="<%=p.getEpFileName() %>" width="110" height="106" /></div>
			<div class="buy">
				商城价：<span class="price"><%=p.getEpPrice() %></span><br />
				库　存：<%if(p.getEpStock()>0){ %>
				<%=p.getEpStock() %>
				<%}else{%>
				无货
				<%} %>
			  <div class="button"><input type="button" name="button" value="" onclick="location.href = 'addCart?epid='" /><a href="shopping.html">放入购物车</a></div>
			</div>
			<div class="clear"></div>
		</div>
		<div class="introduce">
